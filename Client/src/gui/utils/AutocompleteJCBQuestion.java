            ...
            tc.getDocument().addDocumentListener(new DocumentListener() {
                ...
                public void update() {
                    ...
                    SwingUtilities.invokeLater(new Runnable() {
                        @Override
                        public void run() {
                            List<Question> founds = Controller.getInstance().autocompleteQuestions(tc.getText());;
                            setEditable(false);
                            removeAllItems();

                            boolean found = false;
                            for (Question question : founds) {
                                if (tc.getText().equals(question.getText())) {
                                    found = true;
                                    break;
                                }
                            }
                            if (!found) {
                                q.setText(tc.getText());
                                addItem(q);
                            }

                            
                            for (Question s : founds) {
                                addItem(s);
                            }
                            setEditable(true);
